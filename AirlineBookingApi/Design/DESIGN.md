...
The system seeds two demo accounts for testing:

| Role | Email | Password | Account ID |
|------|-------|----------|------------|
| Staff | staff@airline.com | Staff@123 | 1 |
| Customer | customer@airline.com | Customer@123 | 2 |

**Note:** Passwords are BCrypt-hashed (workfactor 11) in the seed data. On a fresh database, run `dotnet ef database update` (or let the app auto-apply migrations on startup via `Database.Migrate()`) to create all tables and seed these accounts.

## Connection String Override

The default `appsettings.json` uses SQL Server LocalDB which works on any machine with Visual Studio or SQL Server Express installed:

```
Server=(localdb)\\MSSQLLocalDB;Database=AirlineBookingDb;Trusted_Connection=True;TrustServerCertificate=True;
```

To use a different SQL Server instance (e.g., your local SQL Express with credentials), override in `appsettings.Development.json`:

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Server=YOUR_SERVER\\SQLEXPRESS;Database=AirlineBookingDb;User Id=sa;Password=YOUR_PASSWORD;TrustServerCertificate=True;"
  }
}
```

`appsettings.Development.json` da duoc .gitignore — copy tu `appsettings.Development.example.json` roi dien thong tin local cua ban.

> **Luu y:** `appsettings.Development.json` da duoc them vao `.gitignore` — hay copy tu `appsettings.Development.example.json` roi dien thong tin ket noi local cua ban.

**Priority:** `appsettings.Development.json` > `appsettings.json` > environment variables > command-line args. The `MSSQLLocalDB` default should work out-of-the-box on machines with Visual Studio.
